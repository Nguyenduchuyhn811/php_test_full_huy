<?php 
	...
	if (!empty($_GET['search_name'])) {
		...
		if (!empty($_GET['type'])) {
			$b = $_GET['type'];
			$sql      = "SELECT * FROM news WHERE name LIKE '%$a%' AND type = $b ORDER BY id DESC LIMIT $start, $per_page";

			$sqlsearch= "SELECT * FROM news WHERE name LIKE '%$a%' AND type = $b ORDER BY id DESC";
			$products_search = mysql($sqlsearch);
		}else{
			$sql      = "SELECT * FROM news WHERE name LIKE '%$a%' ORDER BY id DESC LIMIT $start, $per_page";

			$sqlsearch= "SELECT * FROM news WHERE name LIKE '%$a%' ORDER BY id DESC";
			$products_search = mysql($sqlsearch);
		...
		if (!empty($_GET['type'])) {
			$b = $_GET['type'];
			$sql      = "SELECT * FROM news WHERE type = $b ORDER BY id DESC LIMIT $start, $per_page";

			$sqlsearch= "SELECT * FROM news WHERE type = $b ORDER BY id DESC";
			$products_search = mysql($sqlsearch);
			// echo "<pre>";
	...
	$result_all_product = "SELECT * FROM news ORDER BY id DESC";
				...
				<div class="quantrimenu col-xs-12 col-sm-12 col-md-2 col-lg-2">
					...
					<div class="quantrimenuitem">
						<div class="row">
							...
							<div class="quantriheaderitem col-xs-12 col-sm-12 col-md-12 col-lg-12">
								<a class="active" href="http://localhost:81/occko/webdong/html/quantri/tintuc/danhsachtintuc.php">Danh sách tin tức</a>
							</div>
							<div class="quantriheaderitem col-xs-12 col-sm-12 col-md-12 col-lg-12">
							...
							<div class="item">
								...
								<div class="quantridanhsachsearch">
									...
									<select id="search_type" name="products_type">
										...
										<?php 
											function showCategories($news_type, $parent_id = 0, $char = ''){
												foreach ($news_type as $z => $x) {
										...
										<?php
														// unset($news_type[$z]);
														showCategories($news_type, $x['id'], $char.'---');
													}				
												}
											}
											// hiển thị dao diện
											showCategories($news_type);
										?>
									</select>
									<button>
											...
											<tr>
												...
												<td><?php 
													if ($value['list'] == 0) {
														...
														foreach ($news_type as $key1 => $value1) {
															if ($value['list'] == $value1['id']) {
																echo $value1['name'];
															}
														}
													}
												...
												<td>
													<a href="http://localhost:81/occko/webdong/html/quantri/tintuc/html.php?id=<?php echo $value['id']?>" target="blank" data-id="<?php echo $value['id']?>" ><i class="fa fa-eye" aria-hidden="true"></i></a>
													<a class="refresh" href="http://localhost:81/occko/webdong/html/quantri/tintuc/danhsachtintuc.php?page=<?php echo $huy;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];}?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];}?>" onclick="return confirm('Bạn có chắc làm mới sản phẩm này?')" data-id="<?php echo $value['id']?>" data-alert-title="Thông báo"><i class="fa fa-refresh" aria-hidden="true"></i></a>
													<a class="update" href="http://localhost:81/occko/webdong/html/quantri/tintuc/suadanhsachtintuc.php?id=<?php echo $value['id']?>" target="blank"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
													<a class="delete" href="http://localhost:81/occko/webdong/html/quantri/tintuc/quantrixoatintuc.php?id=<?php echo $value['id']?>" onclick="return confirm('Bạn có chắc xóa sản phẩm này?')" data-id="<?php echo $value['id']?>" data-alert-title="Thông báo"><i class="fa fa-trash" aria-hidden="true"></i></a>
												</td>
											</tr>
										<?php
											}
