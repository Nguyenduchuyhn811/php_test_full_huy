<?php 
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		addAlbums_type($_POST);
	}

	$sql      = "SELECT * FROM albums_type ORDER BY id";
	$albumstype = mysql($sql);
	// echo("<pre>");
	// print_r($products);
				...
				<div class="quantrimenu col-xs-12 col-sm-12 col-md-2 col-lg-2">
					...
					<div class="quantrimenuitem">
						<div class="row">
							...
							<div class="quantriheaderitem col-xs-12 col-sm-12 col-md-12 col-lg-12">
								<a class="active" href="http://localhost/occko/webdong/html/quantri/album/dangdanhmucalbum.php">Đăng danh mục album</a>
							</div>
							<div class="quantriheaderitem col-xs-12 col-sm-12 col-md-12 col-lg-12">
											...
											<div class="row">
												...
												<div class="quantritinhtrang col-xs-12 col-sm-12 col-md-10 col-lg-10">
													<select id="search_type" name="parent_id">
														...
														<?php
															function showCategories($albumstype, $parent_id = 0, $char = ''){
																foreach ($albumstype as $z => $x) {
																	if ($x['parent_id'] == $parent_id){
														?>
																		<option value="<?php echo $x['id'] ?>"><?php echo $char . $x['name'] ?></option>
														<?php
																		// unset($list_type_op[$z]);
																		showCategories($albumstype, $x['id'], $char.'---');
																	}				
																}
															}
															// hiển thị dao diện
															showCategories($albumstype);
														?>
													</select>
												</div>
											</div>
