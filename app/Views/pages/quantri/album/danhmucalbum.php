<?php 
	...
	$mysql    = mysqli_connect('localhost','root','','v2bnc00420');
	
		if (!empty($_GET['type'])) {
			$b = $_GET['type'];
			$sql      = "SELECT * FROM albums_type WHERE type = $b ORDER BY id DESC LIMIT $start, $per_page";
			$sqlsearch= "SELECT * FROM albums_type WHERE type = $b ORDER BY id DESC";;
			$products_search = mysql($result_search);
			// echo "<pre>";
			// print_r($products_search);
		}else{
			$sql      = "SELECT * FROM albums_type ORDER BY id DESC LIMIT $start, $per_page";
			// echo $sql;
		}
	...
	$result_all_albums = "SELECT * FROM albums_type ORDER BY id DESC";
				...
				<div class="quantrimenu col-xs-12 col-sm-12 col-md-2 col-lg-2">
					...
					<div class="quantrimenuitem">
						<div class="row">
							...
							<div class="quantriheaderitem col-xs-12 col-sm-12 col-md-12 col-lg-12">
								<a class="active" href="http://localhost/occko/webdong/html/quantri/album/danhmucalbum.php">Danh mục album</a>
							</div>
						</div>
					</div>
					<div class="quantrimenuitem">
							...
							<div class="item">
								...
								<table class="quantridanhsach table table-hover">
									<thead>
										...
										<th class="discription">Chú thích</th>
									...
									<tbody>
										<?php 
											// print_r($all_albums);
											function showCategories($all_albums, $parent_id = 0, $char = ''){
													foreach ($all_albums as $key => $value) {
										...
										?>
											<tr>
												...
												<td>
													<a class="refresh" href="http://localhost/occko/webdong/html/quantri/album/quanlialbum.php" onclick="return confirm('Bạn có chắc làm mới sản phẩm này?')" data-id="<?php echo $value['id']?>" data-alert-title="Thông báo"><i class="fa fa-refresh" aria-hidden="true"></i></a>
													<a class="update" href="http://localhost/occko/webdong/html/quantri/album/quanlialbum.php?id=<?php echo $value['id']?>" target="blank"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
													<a class="delete" href="http://localhost/occko/webdong/html/quantri/album/quantrixoaalbum.php?idtype=<?php echo $value['id']?>" onclick="return confirm('Bạn có chắc xóa sản phẩm này?')" data-id="<?php echo $value['id']?>" data-alert-title="Thông báo"><i class="fa fa-trash" aria-hidden="true"></i></a>
												</td>
											</tr>
										<?php
													showCategories($all_albums, $value['id'], $char."----");
												}				
											}
										}
										// hiển thị dao diện
										showCategories($all_albums);
										?>
									</tbody>
								</table>
								<div class="quantridanhsachlistbtn">
									<ul>
										<?php if($huy > 1){
											?>
											<li class="disabled">
												<a href="http://localhost/occko/webdong/html/quantri/album/danhmucalbum.php?page=<?php echo $huy-1;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];} ?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>">|<<</a>
											</li>
											<li>
												<a href="http://localhost/occko/webdong/html/quantri/album/danhmucalbum.php?page=<?php echo $huy-1;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];} ?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>"><?php echo $huy - 1;?></a>
											</li>
										<?php
										}else{
										?>
											<li class="disabled">
												<a href="http://localhost/occko/webdong/html/quantri/album/danhmucalbum.php?page=<?php echo $huy;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];}?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>">|<<</a>
											</li>
										<?php
										} 
										;?>						
											<li class="active">
												<a href="http://localhost/occko/webdong/html/quantri/album/danhmucalbum.php?page=<?php echo $huy;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];}?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];}?>"><?php echo $huy;?></a>
											</li>
