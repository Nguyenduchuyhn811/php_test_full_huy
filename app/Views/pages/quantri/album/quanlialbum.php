<?php 
	...
	if (!empty($_GET['search_name'])) {
		...
		if (!empty($_GET['type'])) {
			$b = $_GET['type'];
			$sql      = "SELECT * FROM albums WHERE name LIKE '%$a%' AND type = $b ORDER BY id DESC LIMIT $start, $per_page";

			$sqlsearch= "SELECT * FROM albums WHERE name LIKE '%$a%' AND type = $b ORDER BY id DESC";
			$products_search = mysql($sqlsearch);
		}else{
			$sql      = "SELECT * FROM albums WHERE name LIKE '%$a%' ORDER BY id DESC LIMIT $start, $per_page";

			$sqlsearch= "SELECT * FROM albums WHERE name LIKE '%$a%' ORDER BY id DESC";
			$products_search = mysql($sqlsearch);
		...
		if (!empty($_GET['type'])) {
			$b = $_GET['type'];
			$sql      = "SELECT * FROM albums WHERE type = $b ORDER BY id DESC LIMIT $start, $per_page";

			$sqlsearch= "SELECT * FROM albums WHERE type = $b ORDER BY id DESC";
			$products_search = mysql($sqlsearch);
			// echo "<pre>";
			// print_r($products_search);
		}else{
			$sql      = "SELECT * FROM albums ORDER BY id DESC LIMIT $start, $per_page";
			// echo $sql;
		}
	}
	$albums = mysql($sql);
	...
	$result_all_albums = "SELECT * FROM news ORDER BY id DESC";
	$all_albums = mysql($result_all_albums);
	// echo count($all_products);
	$num_page = (int)(count($all_albums)/$per_page);
	// echo "<br>" . $num_page;
	// echo "<br>" . $num_page*$per_page;
	
	$op = "SELECT * FROM albums_type";
	$albums_type = mysql($op);
				...
				<div class="quantrimenu col-xs-12 col-sm-12 col-md-2 col-lg-2">
					...
					<div class="quantrimenuitem">
						<div class="row">
							...
							<div class="quantriheaderitem col-xs-12 col-sm-12 col-md-12 col-lg-12">
								<a class="active" href="http://localhost/occko/webdong/html/quantri/album/quanlialbum.php">Danh sách album</a>
							</div>
							<div class="quantriheaderitem col-xs-12 col-sm-12 col-md-12 col-lg-12">
							...
							<div class="item">
								...
								<div class="quantridanhsachsearch">
									...
									<script type="text/javascript">
										var a = "http://localhost/occko/webdong/html/danhsach.php";
									...
									<tbody>
										...
										?>
											<tr>
												...
												<td>
													<a href="http://localhost/occko/webdong/html/quantri/tintuc/html.php?id=<?php echo $value['id']?>" target="blank" data-id="<?php echo $value['id']?>" ><i class="fa fa-eye" aria-hidden="true"></i></a>
													<a class="refresh" href="http://localhost/occko/webdong/html/quantri/tintuc/danhsachtintuc.php?page=<?php echo $huy;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];}?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];}?>" onclick="return confirm('Bạn có chắc làm mới sản phẩm này?')" data-id="<?php echo $value['id']?>" data-alert-title="Thông báo"><i class="fa fa-refresh" aria-hidden="true"></i></a>
													<a class="update" href="http://localhost/occko/webdong/html/quantri/tintuc/suadanhsachtintuc.php?id=<?php echo $value['id']?>" target="blank"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
													<a class="delete" href="http://localhost/occko/webdong/html/quantri/tintuc/quantrixoatintuc.php?id=<?php echo $value['id']?>" onclick="return confirm('Bạn có chắc xóa sản phẩm này?')" data-id="<?php echo $value['id']?>" data-alert-title="Thông báo"><i class="fa fa-trash" aria-hidden="true"></i></a>
												</td>
											</tr>
										<?php
											}
								...
								<div class="quantridanhsachlistbtn">
									<ul>
										<?php if($huy > 1){
											?>
											<li class="disabled">
												<a href="http://localhost/occko/webdong/html/quantri/album/quanlialbum.php?page=<?php echo $huy-1;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];} ?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>">|<<</a>
											</li>
											<li>
												<a href="http://localhost/occko/webdong/html/quantri/album/quanlialbum.php?page=<?php echo $huy-1;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];} ?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>"><?php echo $huy - 1;?></a>
											</li>
										<?php
										}else{
										?>
											<li class="disabled">
												<a href="http://localhost/occko/webdong/html/quantri/album/quanlialbum.php?page=<?php echo $huy;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];}?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>">|<<</a>
											</li>
										<?php
										} 
										;?>						
											<li class="active">
												<a href="http://localhost/occko/webdong/html/quantri/album/quanlialbum.php?page=<?php echo $huy;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];}?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];}?>"><?php echo $huy;?></a>
											</li>

										<?php if(count($all_albums) > ($num_page*$per_page) && ($num_page*$per_page) > $start){
											?>
											<li>
												<a href="http://localhost/occko/webdong/html/quantri/album/quanlialbum.php?page=<?php echo $huy+1;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];} ?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>"><?php echo $huy + 1;?></a>
											</li>
