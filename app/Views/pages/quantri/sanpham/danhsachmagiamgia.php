<?php 
	...
	// echo'<br>';
		...
		// if (!empty($_GET['type'])) {
		// 	$b = $_GET['type'];
		// 	$sql      = "SELECT * FROM discount WHERE type = $b ORDER BY id DESC LIMIT $start, $per_page";
		// 	$sqlsearch= "SELECT * FROM discount WHERE type = $b ORDER BY id DESC";;
		// 	$products_search = mysql($result_search);
		// 	// echo "<pre>";
		// 	// print_r($products_search);
		// }else{
		// 	$sql      = "SELECT * FROM discount ORDER BY id DESC LIMIT $start, $per_page";
		// 	// echo $sql;
		// }
	$sql      = "SELECT * FROM discount ORDER BY id DESC LIMIT $start, $per_page";
	$discounts = mysql($sql);
	...
	$result_all_discounts = "SELECT * FROM discount ORDER BY id DESC";
	$all_discounts = mysql($result_all_discounts);
	// echo count($all_products);
	$num_page = (int)(count($all_discounts)/$per_page);
	// echo "<br>" . $num_page;
	// echo "<br>" . $num_page*$per_page;

	// updateDiscount
	// updateDiscount($result_all_discounts,$_POST);
				...
				<div class="quantrimenu col-xs-12 col-sm-12 col-md-2 col-lg-2">
					<div class="quantrimenuitem">
						<div class="row">
							...
							<div class="quantriheaderitem col-xs-12 col-sm-12 col-md-12 col-lg-12">
								<a class="active" href="http://localhost:81/occko/webdong/html/quantri/sanpham/danhsachmagiamgia.php">Danh sách mã giảm giá</a>
							</div>
							<div class="quantriheaderitem col-xs-12 col-sm-12 col-md-12 col-lg-12">
						...
						<div class="quantriitem col-xs-12 col-sm-12 col-md-12 col-lg-12">
							<div class="item">
								...
								<table class="quantridanhsach table table-hover">
									<thead>
										<th class="name">Mã giảm giá</th>
										<th class="discription">Giá trị</th>
										<th class="showoff">Điều kiện</th>
										<th class="">Số lần đã áp dụng</th>
										<th class="option">Hiển thị trang chủ</th>
									...
									<tbody>
										...
										?>
											<tr>
												...
												<td><?php echo number_format($value['value'] ). " " . ($value['type'] == 1 ? 'đ' : '%')?></td>
												<td><?php echo number_format($value['min'])?></td>
												<td><?php echo $value['used'] . "/" . $value['time']; ?></td>
												<td>
								...
								<div class="quantridanhsachlistbtn">
									<ul>
										<?php if($huy > 1){
											?>
											<li class="disabled">
												<a href="http://localhost:81/occko/webdong/html/quantri/sanpham/danhsachmagiamgia.php?page=<?php echo $huy-1;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];} ?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>">|<<</a>
											</li>
											<li>
												<a href="http://localhost:81/occko/webdong/html/quantri/sanpham/danhsachmagiamgia.php?page=<?php echo $huy-1;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];} ?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>"><?php echo $huy - 1;?></a>
											</li>
										<?php
										}else{
										?>
											<li class="disabled">
												<a href="http://localhost:81/occko/webdong/html/quantri/sanpham/danhsachmagiamgia.php?page=<?php echo $huy;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];}?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>">|<<</a>
											</li>
										<?php
										} 
										;?>						
											<li class="active">
												<a href="http://localhost:81/occko/webdong/html/quantri/sanpham/danhsachmagiamgia.php?page=<?php echo $huy;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];}?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];}?>"><?php echo $huy;?></a>
											</li>

										<?php if(count($all_discounts) > ($num_page*$per_page) && ($num_page*$per_page) > $start){
											?>
											<li>
												<a href="http://localhost:81/occko/webdong/html/quantri/sanpham/danhsachmagiamgia.php?page=<?php echo $huy+1;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];} ?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>"><?php echo $huy + 1;?></a>
											</li>
										...
										?>
											<li class="disabled">
												<a href="http://localhost:81/occko/webdong/html/quantri/sanpham/danhsach.php?page=<?php echo $huy;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];}?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>">>>|</a>
											</li>
										<?php
										} 
										;?>
