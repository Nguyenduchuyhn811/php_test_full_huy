<?php 
	...
	// echo'<br>';
		...
		if (!empty($_GET['type'])) {
			$b = $_GET['type'];
			$sql      = "SELECT * FROM video_type WHERE type = $b ORDER BY id DESC LIMIT $start, $per_page";
			$sqlsearch= "SELECT * FROM video_type WHERE type = $b ORDER BY id DESC";;
			$products_search = mysql($result_search);
			// echo "<pre>";
			// print_r($products_search);
		}else{
			$sql      = "SELECT * FROM video_type ORDER BY id DESC LIMIT $start, $per_page";
			// echo $sql;
		}
	...
	$result_all_video = "SELECT * FROM video_type ORDER BY id DESC";
				...
				<div class="quantrimenu col-xs-12 col-sm-12 col-md-2 col-lg-2">
					...
					<div class="quantrimenuitem">
						<div class="row">
							...
							<div class="quantriheaderitem col-xs-12 col-sm-12 col-md-12 col-lg-12">
								<a class="active" href="http://localhost/occko/webdong/html/quantri/video/danhmucvideo.php">Danh mục video</a>
							</div>
						</div>
					</div>
					<div class="quantrimenuitem">
							...
							<div class="item">
								...
								<table class="quantridanhsach table table-hover">
									...
									<tbody>
										<?php 
											// print_r($news);
											function showCategories($all_video, $parent_id = 0, $char = ''){
										...
										?>
											<tr>
												...
												<td>
													<a class="refresh" href="http://localhost/occko/webdong/html/quantri/video/quantridanhmucvideo.php" onclick="return confirm('Bạn có chắc làm mới sản phẩm này?')" data-id="<?php echo $value['id']?>" data-alert-title="Thông báo"><i class="fa fa-refresh" aria-hidden="true"></i></a>
													<a class="update" href="http://localhost/occko/webdong/html/quantri/video/quantrisuadanhmucvideo.php?id=<?php echo $value['id']?>" target="blank"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
													<a class="delete" href="http://localhost/occko/webdong/html/quantri/video/quantrixoavideo.php?idtype=<?php echo $value['id']?>" onclick="return confirm('Bạn có chắc xóa sản phẩm này?')" data-id="<?php echo $value['id']?>" data-alert-title="Thông báo"><i class="fa fa-trash" aria-hidden="true"></i></a>
												</td>
											</tr>
										<?php
													showCategories($all_video, $value['id'], $char."----");
												}				
											}
										}
										// hiển thị dao diện
										showCategories($all_video);
										?>
									</tbody>
								</table>
								<div class="quantridanhsachlistbtn">
									<ul>
										<?php if($huy > 1){
											?>
											<li class="disabled">
												<a href="http://localhost/occko/webdong/html/quantri/video/danhmucvideo.php?page=<?php echo $huy-1;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];} ?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>">|<<</a>
											</li>
											<li>
												<a href="http://localhost/occko/webdong/html/quantri/video/danhmucvideo.php?page=<?php echo $huy-1;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];} ?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>"><?php echo $huy - 1;?></a>
											</li>
										<?php
										}else{
										?>
											<li class="disabled">
												<a href="http://localhost/occko/webdong/html/quantri/video/danhmucvideo.php?page=<?php echo $huy;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];}?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>">|<<</a>
											</li>
										<?php
										} 
										;?>						
											<li class="active">
												<a href="http://localhost/occko/webdong/html/quantri/video/danhmucvideo.php?page=<?php echo $huy;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];}?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];}?>"><?php echo $huy;?></a>
											</li>
