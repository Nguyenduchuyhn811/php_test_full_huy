<?php 
	...
	$sql      = "SELECT * FROM video_type ORDER BY id";
	$videotype = mysql($sql);
	// echo("<pre>");
	// print_r($products);
				...
				<div class="quantrimenu col-xs-12 col-sm-12 col-md-2 col-lg-2">
					...
					<div class="quantrimenuitem">
						<div class="row">
							...
							<div class="quantriheaderitem col-xs-12 col-sm-12 col-md-12 col-lg-12">
								<a class="active" href="http://localhost:81/occko/webdong/html/quantri/video/dangdanhmucvideo.php">Đăng danh mục video</a>
							</div>
							<div class="quantriheaderitem col-xs-12 col-sm-12 col-md-12 col-lg-12">
				...
				<div class="quantrishowoff col-xs-12 col-sm-12 col-md-10 col-lg-10">
					<div class="row">
						<div class="quantriname col-xs-12 col-sm-12 col-md-12 col-lg-12">
							<h1>
								<a href="http://localhost:81/occko/webdong/html/dangdanhmuctintuc.php">
									Đăng danh mục video
								</a>
							</h1>
						</div>
											...
											<div class="row">
												...
												<div class="quantritinhtrang col-xs-12 col-sm-12 col-md-10 col-lg-10">
													<select id="search_type" name="parent_id">
														...
														<?php
															function showCategories($videotype, $parent_id = 0, $char = ''){
																foreach ($videotype as $z => $x) {
																	if ($x['parent_id'] == $parent_id){
														?>
																		<option value="<?php echo $x['id'] ?>"><?php echo $char . $x['name'] ?></option>
														<?php
																		// unset($videotype[$z]);
																		showCategories($videotype, $x['id'], $char.'---');
																	}				
																}
															}
															// hiển thị dao diện
															showCategories($videotype);
														?>
													</select>
												</div>
											</div>
