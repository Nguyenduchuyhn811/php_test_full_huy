<?php 
	...
	$mysql    = mysqli_connect('localhost','root','','v2bnc00420');
		...
		if (!empty($_GET['type'])) {
			$b = $_GET['type'];
			$sql      = "SELECT * FROM news_type WHERE type = $b ORDER BY id DESC LIMIT $start, $per_page";
			$sqlsearch= "SELECT * FROM news_type WHERE type = $b ORDER BY id DESC";;
			$products_search = mysql($result_search);
			// echo "<pre>";
			// print_r($products_search);
		}else{
			$sql      = "SELECT * FROM news_type ORDER BY id DESC LIMIT $start, $per_page";
			// echo $sql;
		}
	...
	$result_all_news = "SELECT * FROM news_type ORDER BY id DESC";
	$all_news = mysql($result_all_news);
	// echo count($all_products);
	$num_page = (int)(count($all_news)/$per_page);
	// echo "<br>" . $num_page;
				...
				<div class="quantrimenu col-xs-12 col-sm-12 col-md-2 col-lg-2">
					...
					<div class="quantrimenuitem">
						<div class="row">
							...
							<div class="quantriheaderitem col-xs-12 col-sm-12 col-md-12 col-lg-12">
								<a class="active" href="http://localhost/occko/webdong/html/quantri/tintuc/danhmuctintuc.php">Danh mục tin tức</a>
							</div>
						</div>
					</div>
					<div class="quantrimenuitem">
							...
							<div class="item">
								...
								<table class="quantridanhsach table table-hover">
									...
									<tbody>
										<?php 
											// print_r($news);
											function showCategories($news, $parent_id = 0, $char = ''){
										...
										?>
											<tr>
												...
												<td>
													<a class="refresh" href="http://localhost/occko/webdong/html/quantri/tintuc/quantridanhmucsp.php" onclick="return confirm('Bạn có chắc làm mới sản phẩm này?')" data-id="<?php echo $value['id']?>" data-alert-title="Thông báo"><i class="fa fa-refresh" aria-hidden="true"></i></a>
													<a class="update" href="http://localhost/occko/webdong/html/quantri/tintuc/quantrisuadanhmucsp.php?id=<?php echo $value['id']?>" target="blank"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
													<a class="delete" href="http://localhost/occko/webdong/html/quantri/tintuc/quantrixoasanpham.php?idtype=<?php echo $value['id']?>" onclick="return confirm('Bạn có chắc xóa sản phẩm này?')" data-id="<?php echo $value['id']?>" data-alert-title="Thông báo"><i class="fa fa-trash" aria-hidden="true"></i></a>
												</td>
											</tr>
										<?php
													showCategories($news, $value['id'], $char."----");
												}				
											}
										}
										// hiển thị dao diện
								...
								<div class="quantridanhsachlistbtn">
									<ul>
										<?php if($huy > 1){
											?>
											<li class="disabled">
												<a href="http://localhost/occko/webdong/html/quantri/tintuc/danhsach.php?page=<?php echo $huy-1;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];} ?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>">|<<</a>
											</li>
											<li>
												<a href="http://localhost/occko/webdong/html/quantri/tintuc/danhsach.php?page=<?php echo $huy-1;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];} ?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>"><?php echo $huy - 1;?></a>
											</li>
										<?php
										}else{
										?>
											<li class="disabled">
												<a href="http://localhost/occko/webdong/html/quantri/tintuc/danhsach.php?page=<?php echo $huy;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];}?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>">|<<</a>
											</li>
										<?php
										} 
										;?>						
											<li class="active">
												<a href="http://localhost/occko/webdong/html/quantri/tintuc/danhsach.php?page=<?php echo $huy;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];}?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];}?>"><?php echo $huy;?></a>
											</li>

										<?php if(count($all_news) > ($num_page*$per_page) && ($num_page*$per_page) > $start){
											?>
											<li>
												<a href="http://localhost/occko/webdong/html/quantri/tintuc/danhsach.php?page=<?php echo $huy+1;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];} ?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];} ?>"><?php echo $huy + 1;?></a>
											</li>
