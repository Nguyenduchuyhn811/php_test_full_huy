<?php 
	...
	// 	if (!empty($_GET['type'])) {
	// 		$b = $_GET['type'];
	// 		$sql      = "SELECT * FROM panertop WHERE name LIKE '%$a%' AND type = $b ORDER BY id DESC LIMIT $start, $per_page";

	// 		$sqlsearch= "SELECT * FROM panertop WHERE name LIKE '%$a%' AND type = $b ORDER BY id DESC";
	// 		$products_search = mysql($sqlsearch);
	// 	}else{
	// 		$sql      = "SELECT * FROM panertop WHERE name LIKE '%$a%' ORDER BY id DESC LIMIT $start, $per_page";

	// 		$sqlsearch= "SELECT * FROM panertop WHERE name LIKE '%$a%' ORDER BY id DESC";
	// 		$products_search = mysql($sqlsearch);
	...
	// 	if (!empty($_GET['type'])) {
	// 		$b = $_GET['type'];
	// 		$sql      = "SELECT * FROM panertop WHERE type = $b ORDER BY id DESC LIMIT $start, $per_page";

	// 		$sqlsearch= "SELECT * FROM panertop WHERE type = $b ORDER BY id DESC";
	// 		$products_search = mysql($sqlsearch);
	// 		// echo "<pre>";
	// 		// print_r($products_search);
	// 	}else{
	// 		$sql      = "SELECT * FROM panertop ORDER BY id DESC LIMIT $start, $per_page";
	// 		// echo $sql;
	// 	}
	// }
	$sql = "SELECT * FROM panertop";
	$qc = mysql($sql);
	// echo count($products)."<br>";

	$result_all_product = "SELECT * FROM panertop ORDER BY id DESC";
	...
	// $op = "SELECT * FROM product_type";
	// $product_type = mysql($op);
				...
				<div class="quantrimenu col-xs-12 col-sm-12 col-md-2 col-lg-2">
					...
					<div class="quantrimenuitem">
						<div class="row">
							...
							<div class="quantriheaderitem col-xs-12 col-sm-12 col-md-12 col-lg-12">
								<a class="active" href="http://localhost:81/occko/webdong/html/quantri/quangcao/quanliqc.php">Danh sách quảng cáo</a>
							</div>
							<div class="quantriheaderitem col-xs-12 col-sm-12 col-md-12 col-lg-12">
									...
									<tbody>
										...
										?>
											<tr>
												...
												<td>
													<a href="http://localhost:81/occko/webdong/html/quantri/quangcao/quanliqc.php?id=<?php echo $value['id']?>" target="blank" data-id="<?php echo $value['id']?>" ><i class="fa fa-eye" aria-hidden="true"></i></a>
													<a class="refresh" href="http://localhost:81/occko/webdong/html/quantri/quangcao/quanliqc.php?page=<?php echo $huy;?>&search=<?php if(!empty($_GET['search'])){ echo $_GET['search'];}?>&type=<?php if(!empty($_GET['type'])){ echo $_GET['type'];}?>" onclick="return confirm('Bạn có chắc làm mới sản phẩm này?')" data-id="<?php echo $value['id']?>" data-alert-title="Thông báo"><i class="fa fa-refresh" aria-hidden="true"></i></a>
													<a class="update" href="http://localhost:81/occko/webdong/html/quantri/quangcao/suadanhsachqc.php?id=<?php echo $value['id']?>" target="blank"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
													<a class="delete" href="http://localhost:81/occko/webdong/html/quantri/quangcao/quantrixoaqc.php?id=<?php echo $value['id']?>" onclick="return confirm('Bạn có chắc xóa sản phẩm này?')" data-id="<?php echo $value['id']?>" data-alert-title="Thông báo"><i class="fa fa-trash" aria-hidden="true"></i></a>
												</td>
											</tr>
										<?php
											}
